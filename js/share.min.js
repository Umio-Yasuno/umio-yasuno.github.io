...
function copy_url(type){const title=document.title;const url=location.href;if(navigator.share){if(type==`title_url`){navigator.share({title:title,url:url});}else{navigator.share({url:url});}
return;}
...
function copy_code(t){const bq=t.parentNode;const code=bq.getElementsByTagName(`code`)[0].textContent;const link=bq.querySelector(`#source-link`).childNodes[0];const txt=(()=>{if(link){return code+`\n\n`+link.text+`\n`+link.href;}else{return code;}})();if(!txt)return;navigator.clipboard.writeText(txt).then(toast_msg());}